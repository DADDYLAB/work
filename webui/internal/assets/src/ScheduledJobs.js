...
export default class ScheduledJobs extends React.Component {
    ...
    fetch() {
        ...
        fetch(`${this.props.url}?page=${this.state.page}`).then((resp) => resp.json()).then((data) => {
            this.setState({
                count: data.count,
                jobs: data.jobs
            });
    ...
    deleteFor(np, jd, jt) {
        fetch(`${this.props.deleteUrl}?job_id=` + jd + `&job_t=` + jt).then(() => {
            this.updatePage(np)
        })
    }
        ...
        return (
            <div className={cx(styles.panel, styles.panelDefault)}>
                ...
                <div className={styles.panelBody}>
                    ...
                    <PageList page={this.state.page} totalCount={this.state.count} perPage={20}
                              jumpTo={(page) => () => this.updatePage(page)}/>
                </div>
                                    ...
                                    <tr key={job.id}>
                                        ...
                                        <td>
                                            <button type="button" className={cx(styles.btn, styles.btnDefault)}
                                                    onClick={() => this.deleteFor(this.state.page, job.id, job.run_at)}> 删除
                                            </button>
                                        </td>
                                    </tr>
                                );
